...
import { cpp } from "@codemirror/lang-cpp"; // C and C++ share similar syntax
...
function CCompiler() {
  ...
  const handleFileCreation = async (e) => {
    ...
    if (response.ok) {
      ...
      setCCode(""); // Reset the code content
  ...
  const handleSave = async (id) => {
    ...
    if (response.ok) {
      ...
      setEditingFile(null);
      setCCode(""); // Reset the editor's content (refresh the editor)
  ...
  const startEditing = (file) => {
    setEditingFile(file);
    setCCode(file.content); // Load content into the editor
  };
